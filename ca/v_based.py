...
class Car:
    ...
    def __init__(self, v = 1, lane = 1):
        ...
        #self.v = 0
        self.v = random.randint(0, Car.v_max[self.size])
    ...
    for i in range(time_period):
        ...
        #for j in (range(car_num)):
            ...
            if k >= car_num or (b_now == 0 and car[j].b == 0) or t_h >= t_s:
                car[j].v = min(car[j].v + 1, Car.v_max[car[j].size])
            ...
            #if (car[j].b == 0 and (\
                #v_now > d or\
                #(dst == 1 and Car.v_max[car[j].size] <= 7))): # slow car right rule
            #if (car[j].b == 0 and (\
                #v_now > d or\
                #dst == 1\
                #)):
...
if __name__ == "__main__":
    #print main()
    #'''
    print('rolen car_l car_r car_n avg_v flow_l flow_r  flow   lanchge    v_l    v_r v_max')
    for v_up in range(1,13):
        Car.v_max = [6, v_up]
        for i in range(0,2):
            #p_car = float(i + 1) / 10
            max_car_num = (i+1)*10
            ...
            print('%4d %5d %5d %5d %6.2f %6.2f %6.2f %5.2f %8d %7.2f %7.2f %5d' 
