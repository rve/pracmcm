...
#define MAXGENS 10000             /* max. number of generations */
...
{
double val;
val = (rand()%(RAND_MAX)/(double)RAND_MAX)*(high - low) + low;
return(val);
}
...
for (mem = 0; mem < POPSIZE; mem++)
	 ...
	 population[mem].fitness += x[i] * sin(sqrt(fabs((double)x[i])));
     // population[mem].fitness += x[i] * x[i];
...
printf("%.9f\n",POPSIZE,population[POPSIZE].fitness);
fclose(galog);
//fclose(output);
//printf("Success\n");
}
